...
bool UEditorValidator_Load::GetLoadWarningsAndErrorsForPackage(const FString& PackageName, TArray<FString>& OutWarningsAndErrors)
{
	...
	if (ExistingPackage && !IsRunningCommandlet() && ShouldAllowFullValidation() && !ExistingPackage->ContainsMap() && !PackageName.EndsWith(TEXT("_BuiltData")))
	{
		...
		if (bSourceFileExists)
		{
			...
			if (ensure(CopyResult == COPY_OK))
			{
				...
				{
					FValidationMessageGatherer::AddIgnorePatterns(InMemoryReloadLogIgnoreList);
					...
					LoadedPackage = LoadPackage(nullptr, *DestPackageName, LoadFlags);
					...
					FValidationMessageGatherer::RemoveIgnorePatterns(InMemoryReloadLogIgnoreList);
				}
				if (LoadedPackage)
				{
