...
bool ASMPlayerCharacter::OnCaught(AActor* TargetActor)
{
	...
	const bool bAttachSuccess = AttachToComponent(TargetCharacter->GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale, TEXT("CatchSlotSocket"));
	...
	CapturedCharcters.Add(TargetCharacter);

	// 캐릭터 상태 위젯을 숨깁니다.
	SetCharacterStateVisibility(false);
	...
	// 위치 보정 무시를 활성화합니다.
	UCharacterMovementComponent* SourceMovement = GetCharacterMovement();
	...
	MulticastRPCApplyAttachOffset();

	// 플레이어의 카메라 뷰를 타겟의 카메라 뷰로 전환합니다.
	APlayerController* SourcePlayerController = Cast<APlayerController>(Controller);
	...
	// 잡히기 GA를 활성화합니다.
	ASC->TryActivateAbilitiesByTag(FGameplayTagContainer(SMTags::Ability::Caught));
