...
#include "SMCharacterSelectionViewModel.h"
...
void USMCharacterSelectionViewModel::InitializeViewModel(UWorld* InWorld)
{
	...
	if (OwningGameState.IsValid())
	{
		...
		UE_MVVM_SET_PROPERTY_VALUE(DefaultTime, OwningGameState->CountdownTime);
		...
		if (const TObjectPtr<APlayerState>& PlayerState = InWorld->GetFirstPlayerController()->PlayerState)
		{
			if (ASMCharacterSelectPlayerState* CharacterSelectPlayerState = Cast<ASMCharacterSelectPlayerState>(PlayerState))
			{
				...
				auto Player1 = NewObject<USMCharacterSelectionEntryViewModel>(this);
		...
		OwningGameState->GetCountdownTimer()->OnCountdownTick.AddDynamic(this, &USMCharacterSelectionViewModel::OnCountdownTick);
	}
}

void USMCharacterSelectionViewModel::OnCountdownTick()
{
	if (OwningGameState.IsValid())
	{
		SetRemainingTime(OwningGameState->GetCountdownTimer()->GetRemainingTime());
	}
}
